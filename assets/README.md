...
Linkwarden currently ships no runtime assets: the upstream image bakes its own
entrypoint (`prisma migrate deploy` + `concurrently` web/worker), and all
config is injected via env vars. This directory exists only to satisfy the
build's non-empty-`assets` requirement.
